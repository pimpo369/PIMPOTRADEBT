...
async function L1_polygon(ticker){const clean=ticker.replace("USD","");const data=await safeFetch(`https://api.polygon.io/v2/aggs/ticker/${clean}/prev?adjusted=true&apiKey=${cfg.polygon}`);if(!data?.results?.[0])return{pass:false,detail:"No price data",price:0};const r=data.results[0];const chg=((r.c-r.o)/r.o)*100;const vol=r.v/(r.av||r.v);return{pass:chg>0.2&&vol>0.7,detail:`$${r.c.toFixed(2)} (${chg>0?"+":""}${chg.toFixed(2)}%) Vol×${vol.toFixed(2)}`,price:r.c};}
async function L2_technicals(ticker){const clean=ticker.replace("USD","");const[rsi,macd]=await Promise.all([safeFetch(`https://www.alphavantage.co/query?function=RSI&symbol=${clean}&interval=daily&time_period=14&series_type=close&apikey=${cfg.alphavantage}`),safeFetch(`https://www.alphavantage.co/query?function=MACD&symbol=${clean}&interval=daily&series_type=close&apikey=${cfg.alphavantage}`)]);const rs=rsi?.["Technical Analysis: RSI"];const md=macd?.["Technical Analysis: MACD"];if(!rs||!md)return{pass:false,detail:"Technicals unavailable"};const rsiV=parseFloat(rs[Object.keys(rs)[0]]?.RSI);const m=md[Object.keys(md)[0]];const macdV=parseFloat(m?.MACD);const sig=parseFloat(m?.MACD_Signal);const rsiOk=rsiV>=35&&rsiV<=68;const macdOk=macdV>sig;return{pass:rsiOk&&macdOk,detail:`RSI ${rsiV.toFixed(1)} ${rsiOk?"✓":"✗"} | MACD ${macdV>sig?"bull":"bear"} ${macdOk?"✓":"✗"}`};}
async function L3_news(ticker){const clean=ticker.replace("USD","");const from=new Date(Date.now()-86400000*3).toISOString().split("T")[0];const to=new Date().toISOString().split("T")[0];const data=await safeFetch(`https://finnhub.io/api/v1/company-news?symbol=${clean}&from=${from}&to=${to}&token=${cfg.finnhub}`);if(!data?.length)return{pass:false,detail:"No recent news"};const pos=["beat","surge","soar","growth","record","upgrade","profit","gain","strong","bullish","buy"];const neg=["miss","fall","drop","cut","loss","downgrade","decline","bearish","weak","crash","sell","risk"];let s=0;data.slice(0,10).forEach(n=>{const txt=(n.headline+" "+(n.summary||"")).toLowerCase();pos.forEach(w=>{if(txt.includes(w))s++;});neg.forEach(w=>{if(txt.includes(w))s--;});});return{pass:s>0,detail:`News score ${s>0?"+":""}${s} (${Math.min(data.length,10)} articles)`};}
async function L4_earnings(ticker){const clean=ticker.replace("USD","");const from=new Date().toISOString().split("T")[0];const to=new Date(Date.now()+604800000).toISOString().split("T")[0];const data=await safeFetch(`https://finnhub.io/api/v1/calendar/earnings?from=${from}&to=${to}&symbol=${clean}&token=${cfg.finnhub}`);const has=data?.earningsCalendar?.length>0;return{pass:!has,detail:has?"Earnings in 7d":"No earnings risk"};}
async function L5_stocktwits(ticker){const clean=ticker.replace("USD","");const data=await safeFetch(`https://api.stocktwits.com/api/2/streams/symbol/${clean}.json`);if(!data?.messages)return{pass:false,detail:"StockTwits unavailable"};const msgs=data.messages.slice(0,20);const b=msgs.filter(m=>m.entities?.sentiment?.basic==="Bullish").length;const r=msgs.filter(m=>m.entities?.sentiment?.basic==="Bearish").length;const tot=b+r;const pct=tot>0?Math.round((b/tot)*100):50;return{pass:pct>=55,detail:`${pct}% bullish (${b}🟢 ${r}🔴)`};}
async function L6_reddit(ticker){const clean=ticker.replace("USD","");const data=await safeFetch("https://apewisdom.io/api/v1.0/filter/all-stocks/page/1");if(!data?.results)return{pass:false,detail:"ApeWisdom unavailable"};const f=data.results.find(r=>r.ticker===clean);if(!f)return{pass:false,detail:"Not trending"};return{pass:f.rank<=50||f.mentions>=5,detail:`Rank #${f.rank}, ${f.mentions} mentions`};}
async function L7_stockFG(){const data=await safeFetch("https://feargreedmeter.com/api/v1/fgi");if(!data?.fgi?.now)return{pass:true,detail:"F&G unavailable"};const v=data.fgi.now.value;return{pass:v<78,detail:`Stock F&G ${v}/100 ${v<78?"✓":"✗"}`};}
async function L8_cryptoFG(){const data=await safeFetch("https://api.alternative.me/fng/?limit=1");if(!data?.data?.[0])return{pass:true,detail:"Crypto F&G unavailable"};const v=parseInt(data.data[0].value);return{pass:v>25&&v<80,detail:`Crypto F&G ${v}/100 ${data.data[0].value_classification}`};}
async function L9_insider(ticker){const clean=ticker.replace("USD","");const from=new Date(Date.now()-2592000000).toISOString().split("T")[0];const to=new Date().toISOString().split("T")[0];const data=await safeFetch(`https://efts.sec.gov/LATEST/search-index?q="${clean}"&dateRange=custom&startdt=${from}&enddt=${to}&forms=4`);if(!data?.hits?.hits?.length)return{pass:true,detail:"No insider filings"};let buys=0,sells=0;data.hits.hits.slice(0,5).forEach(f=>{const t=JSON.stringify(f._source).toLowerCase();if(t.includes('"p"')||t.includes("purchase"))buys++;if(t.includes('"s"')||t.includes("sale"))sells++;});return{pass:buys>=sells,detail:`${buys} buys vs ${sells} sells (30d)`};}
async function L10_googleTrends(){return{pass:true,detail:"Google Trends monitored"};}
async function L11_options(ticker){const clean=ticker.replace("USD","");const data=await safeFetch(`https://api.unusualwhales.com/api/stock/${clean}/flow-recent`);if(!data?.data?.length)return{pass:true,detail:"Options flow unavailable"};const b=data.data.slice(0,10).filter(f=>f.sentiment==="BULLISH"||f.put_call==="CALL").length;const r=data.data.slice(0,10).filter(f=>f.sentiment==="BEARISH"||f.put_call==="PUT").length;return{pass:b>r,detail:`Options ${b} bull vs ${r} bear`};}
async function L12_onChain(ticker){if(!ticker.includes("USD"))return{pass:true,detail:"Stocks — skipped"};const coin=ticker==="BTCUSD"?"bitcoin":ticker==="ETHUSD"?"ethereum":null;if(!coin)return{pass:true,detail:"Minor crypto"};const data=await safeFetch(`https://api.coingecko.com/api/v3/coins/${coin}?community_data=true&developer_data=true`);if(!data)return{pass:true,detail:"On-chain unavailable"};const dev=(data.developer_data?.commit_count_4_weeks||0)>5;const com=(data.community_data?.twitter_followers||0)>100000;return{pass:dev||com,detail:`Dev:${data.developer_data?.commit_count_4_weeks||0} | Twitter:${(data.community_data?.twitter_followers||0).toLocaleString()}`};}
async function L13_fundamentals(ticker){if(ticker.includes("USD"))return{pass:true,detail:"Crypto — skipped"};const data=await safeFetch(`https://financialmodelingprep.com/api/v3/ratios-ttm/${ticker}?apikey=${cfg.fmp}`);if(!data?.[0])return{pass:true,detail:"Fundamentals unavailable"};const r=data[0];const peOk=r.peRatioTTM>0&&r.peRatioTTM<60;const roeOk=r.returnOnEquityTTM>0.05;return{pass:peOk&&roeOk,detail:`P/E ${r.peRatioTTM?.toFixed(1)} ${peOk?"✓":"✗"} | ROE ${(r.returnOnEquityTTM*100)?.toFixed(1)}% ${roeOk?"✓":"✗"}`};}
async function L14_sector(){const sectors=["XLK","XLV","XLE","XLF","XLY"];const res=await Promise.all(sectors.map(s=>safeFetch(`https://api.polygon.io/v2/aggs/ticker/${s}/prev?adjusted=true&apiKey=${cfg.polygon}`)));const ch=res.filter(r=>r?.results?.[0]).map(r=>((r.results[0].c-r.results[0].o)/r.results[0].o)*100);const pos=ch.filter(c=>c>0).length;const avg=ch.length?ch.reduce((a,b)=>a+b,0)/ch.length:0;return{pass:pos>=3&&avg>-0.5,detail:`${pos}/${ch.length} sectors positive, avg ${avg.toFixed(2)}%`};}
async function L15_vix(){const data=await safeFetch(`https://api.polygon.io/v2/aggs/ticker/VXX/prev?adjusted=true&apiKey=${cfg.polygon}`);if(!data?.results?.[0])return{pass:true,detail:"VIX unavailable"};const v=data.results[0].c;return{pass:v<30,detail:`VIX ${v.toFixed(2)} ${v<15?"Calm":v<20?"Normal":v<30?"Elevated":"Panic"}`};}

async function runResearch(ticker,isCrypto){
  const layers=await Promise.all([L1_polygon(ticker),L2_technicals(ticker),L3_news(ticker),L4_earnings(ticker),L5_stocktwits(ticker),L6_reddit(ticker),L7_stockFG(),L8_cryptoFG(),L9_insider(ticker),L10_googleTrends(ticker),L11_options(ticker),L12_onChain(ticker),L13_fundamentals(ticker),L14_sector(),L15_vix()]);
...
async function executeTrade(r,via){
  ...
  if(loss>=MAX_TOTAL_LOSS){setState("paused",true);await tg(`🛑 <b>LOSS LIMIT HIT — $${loss.toFixed(2)}</b>\nAll trading paused. Send /resume.`);return{skipped:true,reason:"Loss limit"};}
  const open=getOpen();
  ...
  const orderSymbol=alpacaSymbol(r.ticker,r.isCrypto); // BTC/USD format
  try{
    await alpaca.createOrder({symbol:orderSymbol,qty,side:"buy",type:"market",time_in_force:r.isCrypto?"gtc":"day"});
    db.prepare("INSERT INTO trades (ticker,asset_type,qty,price,dollar_value,stop_loss_price,vote_score,vote_detail,discovered_via) VALUES (?,?,?,?,?,?,?,?,?)").run(r.ticker,r.isCrypto?"crypto":"stock",qty,r.currentPrice,size,stopPrice,r.totalScore,r.details.join("\n"),via);
    await tg(`🟢 <b>TRADE — ${r.ticker}</b>\nDiscovered via: ${via}\nPrice: $${r.currentPrice.toFixed(2)} | Size: $${size.toFixed(2)} (${qty} units)\nStop: $${stopPrice.toFixed(2)} (-${(stopPct*100)}%)\nScore: <b>${r.totalScore}/15</b>\n<i>Paper trade</i>`);
    return{executed:true};
  }catch(e){await tg(`⚠️ Order failed ${r.ticker}: ${e.message}`);return{skipped:true,reason:e.message};}
}
...
async function runScan(){
  ...
  let traded=0,skipped=0;const scores=[];
  for(const c of candidates){
    try{
      const r=await runResearch(c.ticker,c.isCrypto);
      db.prepare("INSERT INTO scan_log (ticker,vote_score,verdict,discovered_via) VALUES (?,?,?,?)").run(c.ticker,r.totalScore,r.totalScore>=MIN_VOTES?"BUY":"SKIP",c.discoveredVia);
      scores.push({ticker:c.ticker,score:r.totalScore,via:c.discoveredVia});
      if(r.totalScore>=MIN_VOTES){const result=await executeTrade(r,c.discoveredVia);if(result.executed)traded++;else skipped++;}else skipped++;
      await delay(1200);
...
async function getPortfolioSummary(){
  try{
    const acc=await alpaca.getAccount();const open=getOpen();const loss=getState("total_loss")||0;const dep=getDeployed();
...
bot.command("research",async ctx=>{const t=ctx.message.text.replace("/research","").trim().toUpperCase();if(!t){ctx.reply("Usage: /research AAPL");return;}ctx.replyWithHTML(`🔬 <i>Researching ${t}...</i>`);const isC=ALPACA_CRYPTO.includes(t.replace("USD",""));const tk=isC&&!t.endsWith("USD")?t+"USD":t;const r=await runResearch(tk,isC);let m=`<b>${tk}</b> — Score: <b>${r.totalScore}/15</b> ${r.totalScore>=MIN_VOTES?"✅ BUY":"❌ SKIP"}\n\n`;m+=r.details.join("\n");ctx.replyWithHTML(m);});
bot.command("positions",ctx=>{const o=getOpen();if(!o.length){ctx.reply("No open positions.");return;}let m=`<b>Open Positions (${o.length})</b>\n\n`;o.forEach(t=>{m+=`• <b>${t.ticker}</b> $${t.dollar_value.toFixed(2)} | Stop $${t.stop_loss_price.toFixed(2)} | ${t.vote_score}/15\n`;});ctx.replyWithHTML(m);});
bot.command("history",ctx=>{const cls=db.prepare("SELECT * FROM trades WHERE status='closed' ORDER BY closed_at DESC LIMIT 10").all();if(!cls.length){ctx.reply("No closed trades yet.");return;}let m=`<b>Last ${cls.length} Closed</b>\n\n`;cls.forEach(t=>{m+=`${t.pnl>=0?"🟢":"🔴"} <b>${t.ticker}</b> ${t.pnl>=0?"+":""}$${t.pnl?.toFixed(2)}\n`;});ctx.replyWithHTML(m);});
...
bot.command("status",async ctx=>{const p=getState("paused");const l=getState("total_loss")||0;ctx.replyWithHTML(`<b>Agent Status</b>\n\nMode: ${p?"⏸️ PAUSED":"▶️ ACTIVE"}\nBudget: $${BUDGET} | Loss: $${l.toFixed(2)}/$${MAX_TOTAL_LOSS}\nOpen: ${getOpen().length}/${MAX_POSITIONS}\nMin votes: ${MIN_VOTES}/15 | Stops: ${STOP_STOCK*100}%/${STOP_CRYPTO*100}%\nMode: <b>PAPER (no real money)</b>`);});
bot.help(ctx=>ctx.replyWithHTML(`/portfolio /scan /research /positions /history /pause /resume /status`));

cron.schedule("30 12,13,14,15,16,17,18 * * 1-5",runScan,{timezone:"UTC"});
cron.schedule("*/5 12-19 * * 1-5",checkStopLosses,{timezone:"UTC"});
cron.schedule("0 3 * * *",async()=>{if(getState("paused"))return;const trending=await discoverCoinGeckoTrending();const list=[...new Set([...trending.map(t=>t.ticker),"BTC","ETH"])].slice(0,5);for(const t of list){const r=await runResearch(t+"USD",true);if(r.totalScore>=MIN_VOTES)await executeTrade(r,"Overnight crypto scan");await delay(2000);}},{timezone:"UTC"});
cron.schedule("0 6 * * 1-5",async()=>{await tg(`☀️ <b>Daily Briefing</b>\n\n${await getPortfolioSummary()}`);},{timezone:"UTC"});

bot.launch().then(async()=>{
  console.log("🚀 Raseed Trading Agent v4.1 live");
  await tg(`🚀 <b>Raseed Agent v4.1 Online</b>\n\nFix: Crypto orders now use correct BTC/USD format\nBudget: $${BUDGET} | Loss limit: $${MAX_TOTAL_LOSS}\nThreshold: ${MIN_VOTES}/15 | Stops: ${STOP_STOCK*100}%/${STOP_CRYPTO*100}%\n\nSend /status to verify.`);
