// ============================================================
// RASEED AUTONOMOUS TRADING AGENT v4.0
...
//   4. EXECUTE   — buys top scorers passing all guardrails
//
...
const ALPACA_CRYPTO = ["BTC","ETH","AVAX","LINK","LTC","AAVE","BAT","BCH",
...
// (Alpha Vantage NOT used here — saved for L2 technicals)
// ══════════════════════════════════════════════════════════
...
async function discoverPolygonGainers() {
  ...
  return data.tickers.slice(0,15)
    .filter(t => t.ticker && t.ticker.length <= 5)
    .map(t => ({ ticker: t.ticker, source: "Polygon Gainers" }));
}
...
async function discoverPolygonActive() {
  ...
  return data.tickers
    ...
    .slice(0,10)
    .map(t => ({ ticker: t.ticker, source: "Polygon Most Active" }));
}
...
async function discoverFinnhubNewsTrending() {
  ...
  const counts = {};
  const stopwords = ["THE","AND","FOR","WITH","FROM","THIS","THAT","WILL","HAVE","BEEN","NYSE","SEC","CEO","CFO","IPO","ETF","GDP","CPI","USD","EUR","API","NEW","AI","US","UK","EU"];
  data.slice(0,30).forEach(a => {
    const matches = (a.headline + " " + (a.summary||"")).match(/\b([A-Z]{2,5})\b/g) || [];
    matches.forEach(m => {
      if (!stopwords.includes(m)) counts[m] = (counts[m]||0) + 1;
    });
  });
  return Object.entries(counts)
    .sort((a,b) => b[1] - a[1])
    .slice(0,10)
    .map(([t]) => ({ ticker: t, source: "Finnhub News" }));
}
...
async function discoverRedditCrypto() {
  ...
  return data.results.slice(0,15)
    .filter(r => ALPACA_CRYPTO.includes(r.ticker))
    .map(r => ({ ticker: r.ticker, source: "Reddit Crypto" }));
}

async function discoverCoinGeckoTrending() {
  const data = await safeFetch("https://api.coingecko.com/api/v3/search/trending");
  if (!data?.coins) return [];
  const map = { "bitcoin":"BTC","ethereum":"ETH","avalanche-2":"AVAX","chainlink":"LINK",
                "litecoin":"LTC","aave":"AAVE","basic-attention-token":"BAT","bitcoin-cash":"BCH",
                "curve-dao-token":"CRV","dogecoin":"DOGE","polkadot":"DOT","the-graph":"GRT",
                "maker":"MKR","shiba-inu":"SHIB","sushi":"SUSHI","uniswap":"UNI",
                "tezos":"XTZ","yearn-finance":"YFI","ripple":"XRP" };
  return data.coins
    .map(c => ({ ticker: map[c.item.id] || c.item.symbol.toUpperCase(), source: "CoinGecko Trending" }))
    .filter(c => ALPACA_CRYPTO.includes(c.ticker));
}

async function discoverCandidates() {
  console.log("🔍 Running discovery...");
  const [gainers, active, finnhub, redditS, redditC, coinGecko] = await Promise.all([
    discoverPolygonGainers(),
    discoverPolygonActive(),
    discoverFinnhubNewsTrending(),
    discoverRedditStocks(),
    discoverRedditCrypto(),
    discoverCoinGeckoTrending(),
  ]);

  // Score stocks by frequency across sources
  const tickerSources = {};
  [...gainers, ...active, ...finnhub, ...redditS].forEach(c => {
    if (!c.ticker || c.ticker.length > 5) return;
    tickerSources[c.ticker] = tickerSources[c.ticker]
      ? `${tickerSources[c.ticker]}, ${c.source}`
      : c.source;
  });

  const stocks = Object.entries(tickerSources)
    .slice(0,20)
    .map(([ticker]) => ({ ticker, isCrypto: false, discoveredVia: tickerSources[ticker] }));

  // Crypto candidates
  const cryptoSet = new Set(["BTC","ETH"]);
  [...redditC, ...coinGecko].forEach(c => cryptoSet.add(c.ticker));
  const crypto = [...cryptoSet].slice(0,8).map(t => ({
    ticker: `${t}USD`, isCrypto: true,
    discoveredVia: redditC.find(r=>r.ticker===t)?.source ||
                   coinGecko.find(g=>g.ticker===t)?.source || "Crypto baseline"
  }));
...
// L2 is the ONLY layer using Alpha Vantage
// ══════════════════════════════════════════════════════════
...
async function L1_polygon(ticker) {
  ...
  const vol = r.v / (r.av || r.v);
  return { pass: chg > 0.2 && vol > 0.7,
           detail: `$${r.c.toFixed(2)} (${chg>0?"+":""}${chg.toFixed(2)}%) Vol×${vol.toFixed(2)}`,
           price: r.c };
}
...
async function L2_technicals(ticker) {
  ...
  const rs = rsi?.["Technical Analysis: RSI"];
  const md = macd?.["Technical Analysis: MACD"];
  if (!rs || !md) return { pass:false, detail:"Technicals unavailable" };
  const rsiV = parseFloat(rs[Object.keys(rs)[0]]?.RSI);
  const m = md[Object.keys(md)[0]];
  const macdV = parseFloat(m?.MACD), signal = parseFloat(m?.MACD_Signal);
  const rsiOk = rsiV >= 35 && rsiV <= 68;
  const macdOk = macdV > signal;
  return { pass: rsiOk && macdOk,
           detail: `RSI ${rsiV.toFixed(1)} ${rsiOk?"✓":"✗"} | MACD ${macdV>signal?"bull":"bear"} ${macdOk?"✓":"✗"}` };
}
...
async function L3_news(ticker) {
  ...
  data.slice(0,10).forEach(n => {
    ...
    pos.forEach(w => { if(txt.includes(w)) s++; });
    neg.forEach(w => { if(txt.includes(w)) s--; });
  });
...
async function L4_earnings(ticker) {
  ...
  const to    = new Date(Date.now()+604800000).toISOString().split("T")[0];
...
async function L5_stocktwits(ticker) {
  ...
  const tot = b + r;
  const pct = tot > 0 ? Math.round((b/tot)*100) : 50;
...
async function L9_insider(ticker) {
  const clean = ticker.replace("USD","");
  const from  = new Date(Date.now()-2592000000).toISOString().split("T")[0];
  const to    = new Date().toISOString().split("T")[0];
  ...
  let buys=0, sells=0;
  data.hits.hits.slice(0,5).forEach(f => {
    const t = JSON.stringify(f._source).toLowerCase();
    if (t.includes('"p"') || t.includes("purchase")) buys++;
    if (t.includes('"s"') || t.includes("sale")) sells++;
  });
  return { pass: buys >= sells, detail: `${buys} buys vs ${sells} sells (30d)` };
}

async function L10_googleTrends(ticker) {
  return { pass: true, detail: "Google Trends monitored" };
}
...
async function L11_options(ticker) {
  ...
  return { pass: b > r, detail: `Options ${b} bullish vs ${r} bearish` };
}
...
async function L13_fundamentals(ticker) {
  ...
  const r = data[0];
  const peOk  = r.peRatioTTM > 0 && r.peRatioTTM < 60;
  const roeOk = r.returnOnEquityTTM > 0.05;
...
async function L14_sector() {
  ...
  const pos = ch.filter(c => c > 0).length;
...
async function L15_vix() {
  ...
  const v = data.results[0].c;
  const m = v < 15 ? "Calm" : v < 20 ? "Normal" : v < 30 ? "Elevated" : "Panic";
...
async function runResearch(ticker, isCrypto) {
  const layers = await Promise.all([
    ...
    L9_insider(ticker), L10_googleTrends(ticker), L11_options(ticker), L12_onChain(ticker),
    L13_fundamentals(ticker), L14_sector(), L15_vix(),
  ]);
  const names = ["Polygon","Alpha Vantage","Finnhub News","Earnings Risk","StockTwits",
                 "Reddit","Stock F&G","Crypto F&G","SEC Insider","Google Trends",
                 "Options Flow","On-Chain","Fundamentals","Sector Rotation","VIX"];
...
async function executeTrade(r, via) {
  ...
  if (r.totalScore < MIN_VOTES) return { skipped:true, reason:`Only ${r.totalScore}/15 (need ${MIN_VOTES})` };
  ...
  try {
    ...
    db.prepare(`INSERT INTO trades (ticker,asset_type,qty,price,dollar_value,stop_loss_price,vote_score,vote_detail,discovered_via)
                VALUES (?,?,?,?,?,?,?,?,?)`)
      .run(r.ticker, r.isCrypto?"crypto":"stock", qty, r.currentPrice, size,
           stopPrice, r.totalScore, r.details.join("\n"), via);
...
async function checkStopLosses() {
  for (const t of getOpen()) {
    try {
      ...
      const p = data.results[0].c;
      if (p <= t.stop_loss_price) {
        try { await alpaca.closePosition(t.ticker.replace("USD","")); } catch {}
        const pnl = (p - t.price) * t.qty;
...
async function runScan() {
  ...
  await tg(`📡 <i>Discovery scan started — screening market...</i>`);

  const candidates = await discoverCandidates();
  await tg(`🎯 <i>${candidates.length} candidates found — running 15-layer research...</i>`);
  ...
  top5.forEach(t => { msg += `• ${t.ticker}: ${t.score}/15 — via ${t.via}\n`; });
  await tg(msg);
...
bot.command("positions", async ctx => {
  ...
  o.forEach(t => { m += `• <b>${t.ticker}</b> $${t.dollar_value.toFixed(2)}\n  Entry $${t.price.toFixed(2)} | Stop $${t.stop_loss_price.toFixed(2)} | ${t.vote_score}/15\n  <i>via ${t.discovered_via}</i>\n\n`; });
  ctx.replyWithHTML(m);
});
...
bot.help(ctx => ctx.replyWithHTML(
  `/portfolio /scan /research /positions /history /pause /resume /status`
));

// ══════════════════════════════════════════════════════════
// SCHEDULES
// ══════════════════════════════════════════════════════════
// Full scan every 30 min during US market hours (12:30-19:00 UTC)
cron.schedule("30 12,13,14,15,16,17,18 * * 1-5", runScan, { timezone:"UTC" });

// Stop-loss check every 5 minutes during market hours
cron.schedule("*/5 12-19 * * 1-5", checkStopLosses, { timezone:"UTC" });

// Overnight crypto scan — 3am UTC (6am Riyadh)
cron.schedule("0 3 * * *", async () => {
  if (getState("paused")) return;
  const data = await safeFetch("https://api.coingecko.com/api/v3/search/trending");
  const map = {"bitcoin":"BTC","ethereum":"ETH","avalanche-2":"AVAX","chainlink":"LINK",
               "litecoin":"LTC","aave":"AAVE","dogecoin":"DOGE","polkadot":"DOT","ripple":"XRP"};
  const trending = (data?.coins||[])
    .map(c => map[c.item.id])
    .filter(t => t && ALPACA_CRYPTO.includes(t));
  const list = [...new Set([...trending, "BTC", "ETH"])].slice(0, 5);
  for (const t of list) {
...
// Daily morning briefing — 6am UTC (9am Riyadh)
...
bot.launch().then(async () => {
  console.log("🚀 Raseed Trading Agent v4.0 live — Dynamic Discovery + Paper Trading");
  await tg(
    `🚀 <b>Raseed Agent v4.0 Online</b>\n\n` +
    `Mode: Dynamic Discovery + Paper Trading\n` +
    `Budget: $${BUDGET} | Loss limit: $${MAX_TOTAL_LOSS}\n` +
    `Threshold: ${MIN_VOTES}/15 layers must agree\n` +
    `Stops: ${STOP_STOCK*100}% stocks | ${STOP_CRYPTO*100}% crypto\n` +
    `Stop-loss check: every 5 min\n` +
    `Full scan: every 30 min (market hours)\n\n` +
    `Send /status anytime.`
  );
